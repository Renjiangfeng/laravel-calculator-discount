...
class CalculatorDiscount
{
    ...
    public function VerifyRule($condition, $discount_id)
    {
        ...
        $discount      = $discountModel::with(['discount_action', 'discount_rules'])->where('id',
            $discount_id)->first();
        if (!$discount) {
            return false;
        }
        ...
        switch ($discount['discount_rules']['type']) {
            case 'contains_product' : //选择固定商品
                if (is_int($condition['product_id']) || is_string($condition['product_id'])) {
                    $eligible = in_array($condition['product_id'], $configuration['product']) ? true : false;
                } elseif (is_array($condition['product_id'])) {
                    $eligible = array_diff($condition['product_id'], $configuration['product']) == [] ? true : false;
                } else {
                    $eligible = false;
                }
                break;
            case 'contains_category' ://选择固定分类
                if (is_int($condition['category_id']) || is_string($condition['category_id'])) {
                    $eligible = in_array($condition['category_id'], $configuration['category']) ? true : false;
                } elseif (is_array($condition['category_id'])) {
                    $eligible = array_diff($condition['category_id'], $configuration['category']) == [] ? true : false;
                } else {
                    $eligible = false;
                }
                break;
    ...
    public function getDiscountAction($discount_id, $total)
    {
        ...
        $configuration = $discount['discount_action']['configuration'] ? json_decode($discount['discount_action']['configuration'],
            true) : [];
